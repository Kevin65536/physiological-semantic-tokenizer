    ...
    for t, observation in enumerate(values):
        ...
        if t:
            try:
                ...
                exc.diagnostic.update(transition_index=t, update_history=recent_updates,
                    inference_method='joint Student-t quadrature / Gaussian moment matching')
                raise
            covariance = core._project_psd(a@covariance@a.T+q)
        update = dict(time_index=t, observation_mask=mask[t].tolist(),
        ...
        update.update(filtered_mean=mean.tolist(), filtered_covariance=covariance.tolist(),
                      observation_log_likelihood=float(log_likelihood))
        recent_updates = (recent_updates + [update])[-4:]
        if keep_states:
...
class TrajectoryObservationSpec:
    ...
    def matrix(self, input_mask=None):
        ...
        valid = valid.reshape(nout, 3)
        if self.output_mask is not None:
            selected = np.asarray(self.output_mask, dtype=bool)
            if selected.shape != (nout, 3):
                raise ValueError('output mask must match the processing output clock')
            valid &= selected
        return matrix, valid

    def apply(self, values, input_mask=None):
        values = np.asarray(values, dtype=float)
        if values.shape != (len(self.input_time), 3):
            raise ValueError('trajectory values must match the input clock; missing uses NaN')
        mask = np.isfinite(values)
        supplied = np.ones_like(mask)
        if input_mask is not None:
            ...
            mask &= supplied
        if np.any(np.isinf(values) & supplied):
            raise ValueError('visible infinite observations are invalid; missing uses NaN')
        matrix, valid = self.matrix(mask)
        processed = (matrix @ np.where(mask, values, 0.).ravel()).reshape(valid.shape)
        processed[~valid] = np.nan
        return processed

    def with_visible_interpolation(self, input_mask, output_mask):
        """Compile S_out P I_M S_in in the controlled linear feature bridge.

        Interpolation is linear on the actual input clock, with constant
        endpoint extension, matching numpy.interp. A whole missing coordinate
        produces no outputs. Partially observed coordinates require two
        visible samples, as in the native preprocessor. S_out is explicit:
        filled hidden samples are never added as observations by default.

        The returned operator has exactly zero hidden-input columns and zero
        unselected-output rows. The original conservative spec remains intact
        for reproducing v1. This linear map makes no claim about native power,
        optical conversion, or motion correction.
        """
        self.matrix()  # validate operator and clocks before construction
        mask = np.asarray(input_mask, dtype=bool)
        output = np.asarray(output_mask, dtype=bool)
        nin, nout = len(self.input_time), len(self.output_time)
        if mask.shape != (nin, 3) or output.shape != (nout, 3):
            raise ValueError('interpolation masks must match input and output clocks')
        operators = np.zeros_like(self.operators, dtype=float)
        output = output.copy()
        if self.output_mask is not None:
            output &= np.asarray(self.output_mask, dtype=bool)
        clock = np.asarray(self.input_time, dtype=float)
        for channel in range(3):
            visible = np.flatnonzero(mask[:, channel])
            if not len(visible):
                output[:, channel] = False
                continue
            if len(visible) < 2 and len(visible) != nin:
                raise ValueError('insufficient visible samples for interpolation')
            # Columns are visible basis impulses, so hidden values are never
            # inspected; covariance transforms through this same matrix.
            interpolation = np.column_stack([
                np.interp(clock, clock[visible], column)
                for column in np.eye(len(visible))])
            mapped = np.asarray(self.operators)[channel] @ interpolation
            operators[channel][:, visible] = mapped
            operators[channel, ~output[:, channel]] = 0.
        processing = (dict(operation='select_visible_then_linear_interpolation',
            input_mask=mask.tolist(), boundary='constant endpoint extension'),
            *self.processing,
            dict(operation='select_inference_outputs', output_mask=output.tolist(),
                 units='same controlled feature coordinates as the base operator'))
        return TrajectoryObservationSpec(operators, np.array(self.input_time, copy=True),
                                         np.array(self.output_time, copy=True), processing, output)
