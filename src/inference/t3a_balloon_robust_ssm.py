...
``z = (r, s, log(f), log(v), log(p), log(q))``. Finite log states map to
positive compartments, but the drift can reach zero flow in finite time;
the transition checks this domain before integration. Inference is an iterated
...
class FlowDomainExit(FloatingPointError):
    """The unchanged deterministic drift leaves f>0 within this interval.

    This classifies a failed model transition; it does not constrain, project,
    redraw, or repair a state. Filters may attach the preceding update history.
    """

    def __init__(self, diagnostic):
        self.diagnostic = diagnostic
        super().__init__(f"flow_domain_exit at {diagnostic['first_zero_time_s']:.12g} s")


@lru_cache(maxsize=128)
def _flow_drift_matrices(a, beta, kappa, gamma, dt):
    matrix = np.array([[-a, 0., 0.], [beta, -kappa, -gamma], [0., 1., 0.]])
    # |exp(A t) u - u| <= (exp(|A| dt)-I)|u| for 0<=t<=dt.
    # The third component bounds flow displacement. A cheap sufficient
    # certificate avoids root searches for ordinary states, without changing RK4.
    bound = (expm(np.abs(matrix)*dt)-np.eye(3))[2]
    # Augment physical f by a constant, avoiding 1+(f-1) cancellation near zero.
    physical = np.zeros((4, 4))
    physical[:3, :3] = matrix
    physical[1, 3] = gamma
    return physical, bound


def flow_drift_diagnostic(transformed_state, parameters, dt):
    """Matrix-exponential minimum and first zero of the closed (r,s,f) drift.

    Locate *all* extrema, including an interior crossing with positive end
    flow. If F(t)=exp(a*t)*s(t), its derivative is an exponential times a
    sinusoid (underdamped), a linear function (critical), or two real
    ...
    if not np.isfinite(dt) or dt <= 0:
        raise ValueError('flow drift interval must be finite and positive')
    state = _as_vector(transformed_state, _STATE_DIM, 'transformed_state')
    r0, s0, f0 = _transformed_to_physical_unchecked(state)[:3]
    a, beta, gamma = (parameters.fixed.driver_decay_per_s,
                      parameters.fixed.neurovascular_gain, parameters.fixed.gamma)
    kappa = parameters.free.kappa
    matrix, _ = _flow_drift_matrices(a, beta, kappa, gamma, float(dt))
    initial = np.array([r0, s0, f0, 1.])

    def physical(t):
        return expm(matrix*t) @ initial

    velocity0 = beta*r0-kappa*s0-gamma*(f0-1)
    h0 = velocity0+a*s0
    h1 = kappa/2*h0 + (a-kappa)*velocity0-gamma*s0-a*beta*r0
    discriminant = gamma-kappa*kappa/4
    partition = [0., float(dt)]
    if discriminant > 0:
        omega = np.sqrt(discriminant)
        if h0 != 0 or h1 != 0:
            phase = np.arctan2(h1/omega, h0)
            first = int(np.ceil((-phase-np.pi/2)/np.pi))
            last = int(np.floor((omega*dt-phase-np.pi/2)/np.pi))
            partition += [(phase+np.pi/2+j*np.pi)/omega for j in range(first, last+1)
                          if 0 < (phase+np.pi/2+j*np.pi)/omega < dt]
    elif discriminant < 0:
        rate = np.sqrt(-discriminant)
        if h1 != 0:
            ratio = -rate*h0/h1
            if 0 < ratio < 1:
                root = np.arctanh(ratio)/rate
                if root < dt:
                    partition.append(float(root))
    elif h1 != 0 and 0 < -h0/h1 < dt:
        partition.append(float(-h0/h1))

    def roots_on_monotone_intervals(function, knots):
        roots = []
        for left, right in zip(knots[:-1], knots[1:]):
            ...
            if fr == 0:
                roots.append(right)
            if np.signbit(fl) != np.signbit(fr) and fl != 0 and fr != 0:
                roots.append(brentq(function, left, right, xtol=1e-14, rtol=1e-14))
        return roots

    extrema = roots_on_monotone_intervals(lambda t: physical(t)[1], sorted(set(partition)))
    knots = sorted(set([0., float(dt), *extrema]))
    flows = [float(physical(t)[2]) for t in knots]
    zeros = roots_on_monotone_intervals(lambda t: physical(t)[2], knots)
    minimum = int(np.argmin(flows))
    return dict(in_domain=flows[minimum] > 0, minimum_flow=flows[minimum],
        minimum_time_s=knots[minimum], first_zero_time_s=min(zeros) if zeros else None,
        end_flow=flows[-1], interval_s=float(dt), stationary_times_s=extrema,
        initial_transformed_state=state.tolist(),
        method='closed drift matrix exponential with analytic extrema isolation and Brent roots')


def _require_flow_drift_domain(state, parameters, dt):
    r, s, f = _transformed_to_physical_unchecked(state)[:3]
    _, bound = _flow_drift_matrices(parameters.fixed.driver_decay_per_s,
        parameters.fixed.neurovascular_gain, parameters.free.kappa,
        parameters.fixed.gamma, float(dt))
    displacement_bound = bound @ np.abs([r, s, f-1])
    if f > displacement_bound + 32*np.finfo(float).eps*max(1., f, displacement_bound):
        return
    diagnostic = flow_drift_diagnostic(state, parameters, dt)
    if not diagnostic['in_domain']:
        raise FlowDomainExit(diagnostic)
...
def rk4_transition(
    ...
    config.validate()
    state = _as_vector(transformed_state, _STATE_DIM, "transformed_state")
    _require_flow_drift_domain(state, parameters, config.dt)
