    ...
    for i, subject in enumerate(cfg['subjects']):
        for j, modality in enumerate(cfg['curve']['modalities']):
            ...
            for coordinate in cfg['coordinates']:
                ...
                if coordinate == 'broadband_pca' and np.isnan(ys).any():
                    ax.plot(grid[np.isnan(ys)], np.full(np.isnan(ys).sum(), .025), 'x',
                            color='#9f3525', transform=ax.get_xaxis_transform(), clip_on=False)
...
def report(run_dir, cfg, summary, review):
    ...
    lines += ['', '真实状态误差如下，NRMSE按各trial原模型真值的标准差归一化；均为24个独立trial的平均。慢W的似然更高不自动意味着driver更接近真值。', '',
              '| 处理 | r NRMSE，W=0 | r NRMSE，W=−0.5 | clean HbO NRMSE，W=0 | clean HbR NRMSE，W=0 |',
              '| --- | ---: | ---: | ---: | ---: |']
    for variant, row in summary['bridge'].items():
        zero = row['fixed_w']['0.0']['model_truth']
        slow = row['fixed_w']['-0.5']['model_truth']
        lines.append(f"| {names[variant]} | {zero['r']['nrmse']:.3f} | {slow['r']['nrmse']:.3f} | {zero['clean_HbO']['nrmse']:.3f} | {zero['clean_HbR']['nrmse']:.3f} |")
    ...
    lines += ['', '这次固定设定重放中的失败均在正流量下计算出浮点E=1，从而触发严格E<1检查；首例的 `1−E` 仍是有限正数（约6.43×10⁻⁵³、9.21×10⁻²²、6.92×10⁻²⁰）。三个代表失败在8/32子步下仍发生，且事件相对时间不变。因此，这些触发点可定位为极低推断流量处的浮点饱和，不能简单归于两子步积分过粗。**为何观测把推断带到极低流量，仍是另一个模型适配问题**；本轮没有放宽检查、截断状态或重记原失败。', '',
...
def main():
    ...
    parser.add_argument('--render-only', action='store_true', help='render existing numerical review without replay or rewriting it')
    ...
    if args.render_only:
        ...
        if review['source_summary_sha256'] != diagnostic.digest(run_dir/'summary.json'):
            raise ValueError('reviewed summary changed')
    else:
        ...
        (run_dir/'reviewer_snapshot.py').write_bytes(Path(__file__).read_bytes())
