...
def report_step5(run_dir):
    ...
    sensitivity=summary['candidates']['U1_W'].get('numerical_sensitivity',{})
    lines+=['','参数混合与去边界检查同时要求全部 18 个预定 trial 可计算。以下已完成检查的数值不代替缺失案例：','',
            '| 检查 | 已完成数 | 范围 |','|---|---:|---|']
    for key,values in sensitivity.items():
        lines.append(f"| {key} | {len(values)} | [{min(values):.6g}, {max(values):.6g}] |")
    lines+=['','保留的主候选失败：','', '| 被试 / 留出位置 | 错误 |','|---|---|']
    for failure in summary['candidates']['U1_W']['failures']:
        lines.append(f"| {failure['subject']} / {failure['heldout_position']} | {failure['detail'].get('error','未完成')} |")
