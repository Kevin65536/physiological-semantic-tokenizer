...
MEASURED_CONFIG=ROOT/'experiments/configs/physiology_semantic_tokenizer/step5b_v1.yaml'
...
def main():
    ...
    if args.stage=='reference':
        ...
        run_measured(args.measured_config,args.run_dir,args.teacher_run,args.workers)
    else:
        report_unqualified_teacher(args.run_dir)
    return 0
...
def report_unqualified_teacher(run_dir):
    """Add stage reviews and prerequisite decisions to a completed A1 run.

    No measured loader is called. This reports why subsequent experiments
    were not executed; it never relabels a blocked experiment as completed.
    """
    import matplotlib
    matplotlib.use('Agg')
    ...
    if manifest['stage']!='a1' or manifest['execution']!='completed':
        raise ValueError('stage review requires a completed A1 run')
    if summary['qualified_candidates']:
        raise ValueError('qualified candidates require their measured stage; cannot write blocked closure')
    ...
    for axis in cfg['axes']:
        ...
        parameter_information[axis]=dict(material_change=material,mean_interval_width=float(widths.mean()),
                        median_interval_width=float(np.median(widths)),fraction_width_within_material_change=float(np.mean(widths<=material)),
                        ...
                        mle_in_boundary_band_fraction=float(np.mean([p['mle_boundary_distance_fraction']<=cfg['inference']['boundary_band_fraction'] for p in posterior])),
                        individual_physiology_claim=False)
    write_json(run_dir/'parameter_information_diagnostic.json',parameter_information)
    rows=[json.loads(p.read_text()) for p in sorted(run_dir.glob('case_*.json'))]
    failures=[r for r in rows if r.get('execution')=='failed']
    error_counts=Counter(r['error'] for r in failures)
    failure_axes=Counter(r['axis'] for r in failures)
    decisions={stage:dict(stage=stage,execution='not_executed',scientific_verdict='not_evaluated',
                         reason='NO_QUALIFIED_CORE_TEACHER',prerequisite_run=run_dir.name,
                         qualified_candidates=[],measured_subjects_read=[],teacher_exports_created=0)
               for stage in ('b','uq')}
    for stage,decision in decisions.items():write_json(run_dir/f'{stage}_stage_decision.json',decision)
    b_report=[
        '# Step5B 阶段报告：前置资格未满足，真实数据实验未执行','',
        'Step5A1 没有合格 teacher，因此按既定顺序停在真实数据读取之前。本阶段没有 measured 分数、被试参数或真实数据结论；不能称为真实数据验证失败。','',
        '原定设计：subjects 01–18，sessions 01/03/05，每个 session 8 个训练 trial、2 个留出 trial（位置 4、9），检验已有被试和 session 中的新 trial。',
        '候选为固定模型和合成合格的最简单一维模型；中心 4 秒遮挡、整模态缺失、自身历史、任务时序与配对 null 分开比较。','',
        '已完成的工程准备：检查 native/cache 入口；实现 trial 内先遮挡再滤波、功率计算、OD/MBLL 和重采样的纯数组函数，并用四种 mask 的合成 mutation fixture 验证隐藏目标不能影响可见输入。',
        '这些软件检查不等于完整 measured 管线验证。训练尺度、通道/gauge 拟合与 measured 评分没有运行。','',
        '实际数据访问：subjects 01–18、19–23、24–29 的信号数组均未读取；没有新 formal measured campaign，没有 replication 或 protected evaluation。','',
        '未执行的原因来自 `ssm_next.md` 的阶段顺序与冻结 teacher 资格，不是等待额外授权。后续应先解决已观察到的状态/数值/null 失败，再版本化新的验证合同。','']
    (run_dir/'b_stage_report.md').write_text('\n'.join(b_report))
    uq_report=[
        '# 全面 UQ 阶段报告：核心 teacher 未合格，分析未执行','',
        '核心 teacher 资格未成立，全面跨被试/跨模态 UQ、真实数据 calibration、ICC 与精度加权实验均未启动。本阶段不授予潜在状态覆盖或风险排序结论。','',
        'A1 已计算的条件状态方差、参数均值间方差、观测噪声及遮挡 coverage 属于 teacher 资格诊断。它们保留在 A1 结果中，不提升为合格 teacher 的全面 UQ 证据。','',
        'clean teacher 方差仍定义为 E[Var(h(x)|D,phi)] + Var(E[h(x)|D,phi])；仅带噪观测预测再加 Student-t 观测方差。状态后验方差不被改称为纯 epistemic variance。','',
        '未来 measured 校准单位是已知被试的新 trial；leave-one-training-trial-out 结果只能作描述性检查，不声称普通 cross-fit 提供标准 split-conformal 的有限样本保证。','',
        '本轮没有导出可用于训练的合格 teacher，没有训练 tokenizer，也没有测试 precision weighting。原同模态观测空间目标及独立模态输入所有权保持原合同；r 仍仅是共享过程诊断。','']
    (run_dir/'uq_stage_report.md').write_text('\n'.join(uq_report))

    lines=['# Step5 阶段复核','',
           '**Step5A0 完成限定推断校准；Step5A1 完成注册案例核算但未获得合格 teacher；Step5B 和全面 UQ 未执行。整个 Step5 的四个实验阶段没有全部运行。**','',
           f"A0 的 {calibration['independent_cases']} 个独立案例通过注册数值和参数 rank/coverage screen；A1 随后暴露的转移支持域失败意味着该有限面板不能建立全域生成模型有效性。原数值结果保留。",'',
           '| 参数轴 | Oracle 95%覆盖 | 旧评分 95%覆盖 | 联合似然 95%覆盖 | 新 rank KS p | 平均95%区间宽度 / 实质变化尺度 |','|---|---:|---:|---:|---:|---|']
    for axis,v in calibration['axes'].items():
        info=parameter_information[axis]
        lines.append(f"| {axis} | {v['oracle']['coverage95']:.4f} | {v['legacy_generalized_posterior']['coverage95']:.4f} | {v['joint_posterior']['coverage95']:.4f} | {v['joint_posterior']['rank_ks_p']:.5f} | {info['mean_interval_width']:.4f} / {info['material_change']:.4f} |")
    lines+=['','区间宽度、边界质量、likelihood-only 与 MAP 分离诊断见 [参数信息诊断](parameter_information_diagnostic.json)。校准和参数可辨识性分开判读；此处不授予个体生理解释。','',
           f"A1 预定 {summary['independent_cases']} 个案例，成功 {summary['successful_cases']}，失败 {summary['failed_cases']}；按轴失败数：{dict(sorted(failure_axes.items()))}。",
           '同一随机种子没有重抽，失败案例没有替换。下表对成功子集的均值和区间仅作描述，不能消除选择偏差。','',
           '| 错误 | 案例数 |','|---|---:|']
    lines += [f'| {error} | {count} |' for error,count in sorted(error_counts.items())]
    audit_path=run_dir/'generation_support_audit.json'
    if audit_path.exists():
        audit=json.loads(audit_path.read_text())
        lines+=['',f"对同一注册 main/donor/task-template 随机流的补充审计：{audit['attempted_streams']} 条生成轨迹中 {audit['failed_streams']} 条失败。这些轨迹在参数 replicate 内相关，不能当作新的独立 SBC 样本。",'',
                '| 参数轴 / replicate | stream | 真值 | 2 / 8 / 32 子步停止时间（秒） |','|---|---:|---:|---|']
        for row in audit['failures']:
            lines.append(f"| {row['axis']} / {row['replicate']:02d} | {row['stream']} | {row['truth']:.5f} | "+' / '.join(f"{r['stop_time_s']:.2f}" for r in row['refinements'])+' |')
        lines+=['','这些失败在子步加密后仍出现；子步变化仅作同种子定位，改变离散转移，不用于重判原案例。详见 [生成支持审计](generation_support_audit.json)。',
                '另一个固定输入定位到 U1_G 的 matched / center_EEG / own_history_and_task 条件；2、8、32 子步均触发氧提取严格数值域异常。见 [null 输入诊断](G04_null_failure_diagnostic.json)。']
    lines+=['','| 候选 | 成功主案例 | 未通过的冻结/完整性检查 |','|---|---:|---|']
    lines += [f"| {c} | {v['independent_cases']} | {', '.join(k for k,ok in v['checks'].items() if not ok)} |" for c,v in summary['candidates'].items()]
    lines+=['','| 候选 | 分支 | r NRMSE | r corr | r 95%覆盖 | EEG | HbO | HbR |','|---|---|---:|---:|---:|---:|---:|---:|']
    for candidate,result in summary['candidates'].items():
        for law,label in [('matched_model_calibration','matched'),('misspecification_stress_test','stress')]:
            if law not in result['laws']:continue
            state=result['laws'][law]['state']
            lines.append(f"| {candidate} | {label} | {state['r']['nrmse']['mean']:.4f} | {state['r']['correlation']['mean']:.4f} | "+' | '.join(f"{state[t]['coverage95']['mean']:.4f}" for t in localization.TARGETS)+' |')
    lines+=['','| 候选 | driver corr q05（均值） | driver NRMSE q95（均值） | 参数混合 mean 加密差/噪声 | 参数混合 variance 相对加密差 |','|---|---:|---:|---:|---:|']
    for candidate,result in summary['candidates'].items():
        if not result['laws']:continue
        stable=result['laws']['matched_model_calibration']['stability']
        lines.append(f"| {candidate} | {stable['driver_correlation_q05']['mean']:.5f} | {stable['driver_nrmse_q95']['mean']:.5f} | {result.get('maximum_mixture_mean_difference_in_noise_units','—')} | {result.get('maximum_mixture_relative_variance_difference','—')} |")
    lines+=['','U0 固定参数敏感性：`'+json.dumps(summary['candidates']['U0_FIXED'].get('sensitivity',{}),ensure_ascii=False)+'`。','',
            '完整双向 null 配对区间见 [A1 主报告](summary.md)；整模态 coverage、方差分解和 U3 诊断见 [结构化汇总](summary.json)。',
            '失配 stress 的 coverage 不称 SBC；参数/状态的数值积分收敛不能替代共享信息增量。','',
            '前一次执行在首个异常后等待进程池退出，没有保存案例结果；其 manifest、source snapshot 和异常记录保留在 `20260907_a1_teacher_v1`。本次仅修正逐案例异常记录及进程调度，科学函数、配置和种子保持一致。','',
            '后续阶段分别见 [Step5B 未执行报告](b_stage_report.md) 与 [全面 UQ 未执行报告](uq_stage_report.md)。','']
    (run_dir/'stage_review.md').write_text('\n'.join(lines))

    candidates=[c for c,v in summary['candidates'].items() if v['laws']]
    colors=['#276FBF','#369A7E','#DD8452','#8D6CAB']
    fig,axes=plt.subplots(1,3,figsize=(16,4.7),layout='constrained')
    labels=['r','clean EEG','clean HbO','clean HbR'];x=np.arange(4)
    for i,candidate in enumerate(candidates):
        state=summary['candidates'][candidate]['laws']['matched_model_calibration']['state']
        means=[state[t]['coverage95']['mean'] for t in localization.TARGETS]
        intervals=np.array([state[t]['coverage95']['ci95'] for t in localization.TARGETS])
        axes[0].errorbar(x+(i-1.5)*.12,means,yerr=np.maximum(np.array([means-intervals[:,0],intervals[:,1]-means]),0),fmt='o',label=candidate,color=colors[i],capsize=3)
    axes[0].axhline(.95,color='#333333',ls='--',lw=1);axes[0].axhspan(.90,.995,color='#aaaaaa',alpha=.12)
    axes[0].set(xticks=x,xticklabels=labels,ylabel='Mean coverage (95% interval)',title='Successful subset: latent / clean coverage')
    axes[0].legend(fontsize=8)
    controls=cfg['teacher']['controls']
    for ax,mask,title in zip(axes[1:],cfg['teacher']['primary_masks'],['Predict masked EEG','Predict masked fNIRS']):
        for i,candidate in enumerate(candidates):
            scores=summary['candidates'][candidate]['laws']['matched_model_calibration']['shared_increments'][mask]
            means=np.array([scores[c]['mean'] for c in controls]);ci=np.array([scores[c]['ci95'] for c in controls])
            ax.errorbar(x+(i-1.5)*.12,means,yerr=np.maximum(np.array([means-ci[:,0],ci[:,1]-means]),0),fmt='o',color=colors[i],capsize=3)
        ax.axhline(0,color='#333333',ls='--',lw=1)
        ax.set(xticks=x,xticklabels=['History','History + task','Pairing null','Shift null'],title=title,ylabel='Joint minus control log score')
        ax.tick_params(axis='x',labelrotation=20)
    fig.suptitle(f"Step5A1: {summary['successful_cases']}/{summary['independent_cases']} complete cases; no teacher qualified",fontweight='bold')
    for suffix in ('png','pdf'):fig.savefig(run_dir/f'teacher_coverage_and_nulls.{suffix}',dpi=180)
    plt.close(fig)
    provenance=dict(stage='post_run_stage_review',execution='completed',
                    created_at=datetime.now(timezone.utc).isoformat(),
                    source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
                    inputs={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest()
                            for p in (run_dir/'summary.json',run_dir/'manifest.json',calibration_dir/'summary.json',audit_path,run_dir/'G04_null_failure_diagnostic.json') if p.exists()},
                    b=decisions['b'],uq=decisions['uq'],measured_data_read=False)
...
def load_measured_config(path=MEASURED_CONFIG):
    ...
    if spec['schema']!='step5b_measured_v1':raise ValueError('wrong measured schema')
    loaded=[]
...
def fit_measured_projection(features,base,spec):
    ...
    pair_scores=(signal/difference).mean(axis=1);pair=int(np.argmax(pair_scores))
    ...
    return dict(eeg_center=center,eeg_feature_scale=scale,pca_center=pca_center,loading=loading,
                ...
                pair_scores=pair_scores,pca_explained_fraction=float(singular[0]**2/np.sum(singular**2)),
...
def prepare_measured_subject(base,spec,metadata,subject,noise_constant):
    ...
    for record in records:
        ...
        for position,event in enumerate(events):
            ...
            if not np.isfinite(e).all() or not np.isfinite(f).all() or np.any(f<=0):
    ...
    projection=fit_measured_projection([features[i] for i in train],base,spec)
...
def measured_trial_case(base,spec,subject,position,detail,arrays,config,fit):
    ...
    row=dict(subject=subject,heldout_position=position,identity=identity,execution='completed',candidates={});payload={'observations':y}
...
def measured_summary(base,spec,rows,fit,preparation_failures,metadata_summary):
    from collections import Counter
    failures=[r for r in rows if r['execution']=='failed'];successes=[r for r in rows if r['execution']=='completed']
    ...
    for candidate in spec['candidates']:
        ...
        checks['complete_registered_subjects_and_trials']=len(complete_subjects)==q['complete_subjects'] and len(successes)==108 and not failures and not preparation_failures
    ...
    summary['primary_minus_fixed_score']={mask:subject_stat(lambda r:r['candidates'][spec['primary']]['masks'][mask]['joint_score']-r['candidates']['U0_FIXED']['masks'][mask]['joint_score']) for mask in base['teacher']['masks']}
    return summary


def measured_report(base,spec,summary):
    lines=['# Step5B 阶段报告：已知被试与 session 中的新 trial','',
           f"注册 18 被试 × 6 留出 trial = 108；完成 {summary['successful_heldout_trials']}，失败 {summary['failed_heldout_trials']}；具有完整六条留出结果的被试 {len(summary['complete_subjects'])}。",'',
...
def run_measured(config_path,run_dir,teacher_dir,workers=None):
    ...
    prepared={};preparation_failures={};rows=[]
    try:
        ...
        with ProcessPoolExecutor(max_workers=min(workers,4),mp_context=get_context('spawn')) as pool:
            jobs={pool.submit(prepare_measured_subject,base,spec,metadata,subject,noise_check['difference_mad_constant']):subject for subject in base['measured']['subjects']}
            for job in as_completed(jobs):
                subject=jobs[job]
                try:
                    detail,arrays,config=job.result();prepared[subject]=dict(detail=detail,arrays=arrays,config=config)
        ...
        fit,fit_failures=fit_measured_posteriors(base,spec,prepared,workers,run_dir)
