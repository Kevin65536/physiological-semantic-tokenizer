...
def fit_measured_projection(features,base,spec,pair_eligible=None):
    ...
    if pair_eligible is not None:
        ...
        pair_scores=np.where(pair_eligible,pair_scores,-np.inf)
    pair=int(np.argmax(pair_scores))
...
def run_measured(config_path,run_dir,teacher_dir,workers=None,reuse_dir=None):
    ...
    reuse_manifest=None;reuse_summary=None
    if reuse_dir:
        ...
        if compatible!=old or reuse_manifest['execution']!='completed' or reuse_manifest['stage']!='b':
    ...
    try:
        ...
        if reuse_dir:
            ...
            for subject in base['measured']['subjects']:
                ...
                else:preparation_failures[subject]=reuse_summary['preparation_failures'][subject]
            manifest.update(reuse_run=str(reuse_dir.relative_to(ROOT)),reused_prepared_subjects=sorted(prepared),
                            reuse_inputs_sha256=reuse_hashes,case_predictions_recomputed_after_local_refinement=True)
        ...
        fit,fit_failures=fit_measured_posteriors(base,spec,{s:v for s,v in prepared.items() if s in new_subjects},workers,run_dir)
        fit.update(reused_fit)
        preparation_failures.update(fit_failures)
        if 'local_refinement' in spec['posterior']:
            fit=refine_measured_posterior_mass(base,spec,prepared,fit,workers,run_dir)
        with ProcessPoolExecutor(max_workers=workers,mp_context=get_context('spawn')) as pool:
            jobs={}
            for subject in base['measured']['subjects']:
